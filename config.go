...
func dumpConfig(config tomlConfig) {
	...
	for i, reading := range config.TemplateKV.Readings {
		fmt.Printf("    {holding_register = %d, size = \"%s\", param_name = \"%s\"}", reading.HoldingRegister, reading.Size, reading.ParamName)
...
func generateDefaultConfig(filename string) {
	...
	f.WriteString("    {holding_register = 40001, size = \"UINT32\", param_name = \"my_param_name_1\"},\n")
	f.WriteString("    {holding_register = 40003, size = \"UINT32\", param_name = \"my_param_name_2\"},\n")
	f.WriteString("    {holding_register = 40005, size = \"UINT32\", param_name = \"my_param_name_3\"}\n")
	f.WriteString("]\n")
